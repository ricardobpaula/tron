...
import br.com.ricardo.tron.model.GrupoUsuario;
...
public class UsuariosImpl implements UsuariosQueries {
	...
	public Optional<Usuario> porLoginEAtivo(String login) {
		...
		return manager
				.createQuery("from Usuario where lower(login) = lower(:login) and ativo = true", Usuario.class)
		...
		if (filtro != null) {
			...
			if (filtro.getGrupos() != null && !filtro.getGrupos().isEmpty()) {
				...
				for (Long idGrupo : filtro.getGrupos().stream().mapToLong(Grupo::getId).toArray()) {
					DetachedCriteria dc = DetachedCriteria.forClass(GrupoUsuario.class);
